...
**2026-06-26** — 生产操作手册已更新 ①⑧ 飞书记录入库 + ④⑤ 成本测算。模型选型结论：保持 DeepSeek V4-Flash，不切 Qwen。6 个新选题待跑 ②→⑮，管线未启动。**项目已从 lab 搬入 `D:\workspace\猫波信号站\`**，`.project` 身份证已创建（id: `catwave-signal-station`），10 个文件的旧路径指针已全部更新。
